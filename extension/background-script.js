// Background script for the OBS Multi-Platform Chat Overlay extension
...
class SocketManager {
  ...
  sendChatMessage(message) {
    if (this.isConnected && this.socket) {
      ...
      console.log('Message sent:', message.platform, message.username, message.id.substring(0, 12) + '...');
      return true;
    } else {
      ...
      tabs.forEach(tab => {
        ...
        if (tab && tab.url && (tab.url.includes('youtube.com') || tab.url.includes('twitch.tv')) && readyTabs.has(tab.id)) {
          chrome.tabs.sendMessage(tab.id, {
            ...
            console.log(`Tab ${tab.id} not ready for messages yet:`, error);
    ...
    if (socketManager.isConnected && sender.tab && sender.tab.id) {
      setTimeout(() => {
        chrome.tabs.sendMessage(sender.tab.id, {
          ...
          console.log(`Tab ${sender.tab.id} failed to receive connection status:`, error);
        });
      }, 500);
    }
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  console.log('OBS Multi-Platform Chat Overlay extension installed');
});
